...
def run_phase_0b(model_id_7b: str):
    ...
    # Generate baseline NLL (all-Q4)
    print("\n  Computing all-Q4 baseline NLL...")
    baseline_nlls = _compute_generation_nll(model, inner, tokenizer, tokens, n_tokens=20)
    ...
    # Generate Q2 NLL
    print("  Computing 3-block-Q2 NLL...")
    q2_nlls = _compute_generation_nll(model, inner, tokenizer, tokens, n_tokens=20)
...
def _compute_generation_nll(model, inner, tokenizer, prompt_tokens, n_tokens=20):
    """Generate tokens and compute per-token NLL."""
    ...
    for i in range(n_tokens):
        # NLL = -log_softmax(logits)[actual_next_token]
        last_logits = logits[0, -1, :]
        log_probs = mx.log_softmax(last_logits)
        next_token = mx.argmax(last_logits).item()

        if i > 0:  # skip first token (determined by prompt)
